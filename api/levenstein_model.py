import sys
sys.path.append('../')
from string_distance_levenshtein.levenshtein import levenshtein
...
def get_similar_entity(nama_instansi, reference_version='v2'):
    ...
    if len(similar_phrases_dict) != 0:
        similar_entity = min(similar_phrases_dict, key=similar_phrases_dict.get)
        official_instansi = get_official_instansi(true_pairs, similar_entity)
        return official_instansi
    else:
